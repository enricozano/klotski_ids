...
class HRSolution:
    ...
    def _output_graphical(self, results_per_line, step_iter, step_wid=2):
        ...
        while True:
            ...
            for i in range(results_per_line):  # Read n steps
                ...
                graphic_move_list.append(
                    self.init_board.get_board())  # append a board, e.g. ['1 0 0 2', '1 0 0 2', '3 5 5 4' ... ]
    ...
    def outputToFile(self, filename, results_per_line=5, graphical=False, dir_shorthand=False):
        with open(filename, 'w') as file:
            ...
            if graphical:
                results_per_line = min(results_per_line, 5)
                self._output_graphicalToFile(file, results_per_line, step_iter, step_wid)
            else:
                results_per_line = min(results_per_line, 7)
                self._output_movesToFile(file, results_per_line, step_iter, step_wid)




    def _output_movesToFile(self, file, results_per_line, step_iter, step_wid):
        for i, s in step_iter:
            file.write("{1}\n".format(i+1, str(s)))

    def _output_graphicalToFile(self, file, results_per_line, step_iter, step_wid):
        init_hash = self.init_board.hash_board()
        for i, s in step_iter:
            file.write("#{0} {1}\n".format(i+1, str(s)))
            graphic_move_list = []
            for _ in range(results_per_line):
                if s is None:
                    break
                self.init_board.dehash_board(s.hash_code)
                graphic_move_list.append(self.init_board.get_board())
                i, s = next(step_iter, (None, None))
